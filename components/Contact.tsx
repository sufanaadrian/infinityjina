...
// ─── Ultramodern — underline-only minimalist form + gold-labeled info list ──
function UltraInfoRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="py-5 border-t border-white/[0.08]">
      <p className="text-[10px] tracking-[0.35em] uppercase text-gold-70 mb-2">{label}</p>
      <div className="text-white/80 font-light">{children}</div>
    </div>
  );
}
...
function ContactUltramodern({
  ...
  return (
    <section id="contact" className="py-28 md:py-36 bg-[#0A0A0B]">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className={showForm ? "grid lg:grid-cols-2 gap-14 lg:gap-20" : "max-w-2xl"}>
          ...
          <div className="flex flex-col border-b border-white/[0.08]">
            ...
            {business.email && (
              <UltraInfoRow label="Email">
                <a href={`mailto:${business.email}`} className="hover:text-gold transition-colors break-all">{business.email}</a>
              </UltraInfoRow>
            )}
            {address && <UltraInfoRow label="Adresă">{address}</UltraInfoRow>}
            {business.hours && (
