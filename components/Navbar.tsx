...
// ─── Ultramodern — ultra-thin transparent header, gold hairline, blur on scroll ─
function NavbarUltramodern() {
  ...
  const headerCls = overlay
    ? "fixed top-0 left-0 right-0 z-50 bg-transparent transition-colors duration-500"
    : "fixed top-0 left-0 right-0 z-50 bg-black/70 backdrop-blur-md border-b border-gold-soft transition-colors duration-500";
  const navLinkCls = "text-white/55 hover:text-gold transition-colors";

  return (
    <header className={headerCls} style={{ paddingTop: "env(safe-area-inset-top)" }}>
      <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
        <NavLogo textCls={`font-light ${textSizeCls} tracking-[0.3em] uppercase text-white`} />
        <nav className="hidden md:flex items-center gap-10 text-xs font-light tracking-[0.25em] uppercase">
          <a href="#services" className={navLinkCls}>Servicii</a>
          {isAccommodation && <a href="#rooms" className={navLinkCls}>Camere</a>}
          <a href="#about" className={navLinkCls}>Despre noi</a>
          <a href="#contact" className={navLinkCls}>Contact</a>
        ...
        <div className="flex items-center gap-3">
          ...
          <details ref={detailsRef} className="md:hidden relative">
            ...
            <nav className="absolute top-full right-0 mt-1 w-48 bg-black/95 backdrop-blur-md border border-gold-soft shadow-xl px-5 py-4 flex flex-col gap-3 text-sm font-light tracking-wide text-white/70">
              <a href="#services" onClick={closeMenu} className="hover:text-gold transition-colors py-1">Servicii</a>
...
export default function Navbar() {
  ...
  if (v === "ultramodern") return <NavbarUltramodern />;
