...
export default function Services() {
  ...
  if (v === "ultramodern") return <ServicesUltramodern {...props} />;
  return <ServicesClassic {...props} />;
}

// ─── Ultramodern — editorial index: sticky header + outline-numeral rows ────
function ServicesUltramodern({ services, headingTitle, headingSubtitle }: SectionProps) {
  return (
    <section id="services" className="py-28 md:py-36 bg-[#0A0A0B]">
      <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-[0.85fr_1.15fr] gap-12 md:gap-16">
        <div className="md:sticky md:top-28 md:self-start">
          <UltraHeading eyebrow="Servicii" title={headingTitle} subtitle={headingSubtitle} align="left" />
        </div>
        <div className="border-t border-white/[0.08]">
          {services.map((service, index) => (
            <Reveal
              key={index}
              delay={(index % 4) * 0.06}
              className="group relative border-b border-white/[0.08] py-7 md:py-8 flex items-start gap-5 md:gap-9 transition-[padding] duration-500 md:hover:pl-3"
            >
              <span className="um-outline-num text-4xl md:text-6xl font-extralight leading-none tabular-nums shrink-0 w-14 md:w-20">
                {String(index + 1).padStart(2, "0")}
              </span>
              <div className="flex-1 pt-1">
                <div className="flex items-baseline justify-between gap-4">
                  <h3 className="text-xl md:text-2xl font-light tracking-wide text-white transition-colors group-hover:text-gold">
                    <RichInline text={service.title} />
                  </h3>
                  {service.price && (
                    <span className="text-sm font-medium tracking-wider text-gold shrink-0">{service.price}</span>
                  )}
                </div>
                <RichText text={service.description} className="mt-2.5 text-white/45 text-sm font-light leading-relaxed max-w-xl" />
              </div>
            </Reveal>
          ))}
        </div>
