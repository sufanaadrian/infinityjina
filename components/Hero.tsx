...
// Whether any hero photo exists (landscape or portrait-only).
const hasHeroImage = !!(business.heroImageUrl || business.heroImagePortraitUrl);

// Renders the hero photo, switching between a landscape image (larger screens)
// and an optional portrait crop (phones) so a wide photo isn't zoomed/cropped
// on a tall screen. When both are set we use a <picture> for true art direction
// — each device downloads only the one image it needs. With a single image we
// keep next/image's optimization (responsive srcset + priority/lazy).
function HeroPhoto({
  ...
  const land = business.heroImageUrl;
  const port = business.heroImagePortraitUrl;
  const alt = business.heroImageAlt ?? stripMarkdown(business.name);

  if (land && port) {
    return (
      <picture>
        <source media="(max-width: 767px)" srcSet={port} />
...
function getNextSectionColor(variant: string): string {
  if (variant === "nature")
    return business.natureBackgroundColor || DEFAULT_NATURE_BG;
...
function getOverlay(
  ...
  if (isNature) {
    ...
    return {
      className: "",
      style: { backgroundColor: hexToRgba(getNatureAccentColor(), opacity) },
    };
  }
  if (heroOverlay) {
...
function HeroSplit({ variant }: { variant: "classic" | "nature" | "dark" }) {
  ...
  const overlay = getOverlay(variant, business.heroOverlay, business.heroTint);
  ...
  return (
    ...
    >
      ...
      <div className="relative order-1 md:order-2 md:w-1/2 h-[40vh] md:h-auto">
        {hasHeroImage ? (
          <>
            <HeroPhoto className="object-cover" />
            <div
...
function HeroNature() {
  const ctaRadius = getButtonRadius(false, business.buttonStyle);
  const overlay = getOverlay("nature", business.heroOverlay, business.heroTint);
  ...
  return (
    <section
      ...
      style={{ backgroundColor: getNextSectionColor("nature") }}
    >
      <div className="relative flex-1 min-h-[55vh] md:min-h-[60vh]">
        {hasHeroImage ? (
          ...
          <div
            className="absolute inset-0"
            style={{ backgroundColor: getNatureAccentColor() }}
          />
        )}
        <div
...
function HeroDark() {
  const ctaRadius = getButtonRadius(false, business.buttonStyle);
  const overlay = getOverlay("dark", business.heroOverlay, business.heroTint);
...
function HeroBold() {
  const overlay = getOverlay("bold", business.heroOverlay, business.heroTint);
...
// ─── Ultramodern — full-bleed cinematic photo, gold script brand, minimalist ─
function HeroUltramodern() {
  const overlay = getOverlay(
    "ultramodern",
    business.heroOverlay,
    business.heroTint,
  );
  const nextColor = getNextSectionColor("ultramodern");
  const hasBackdrop = business.heroTextBackdrop === true;

  return (
    <section className="relative min-h-screen min-h-[100dvh] flex items-center justify-center bg-[#0A0A0B] text-white">
      {/* Photo wrapper extends above the section by env(safe-area-inset-top) so
          the image fills behind the iOS status bar. The section itself has no
          overflow-hidden so this upward extension is not clipped; the kenburns
          scale overflow is still contained by the wrapper's own overflow-hidden. */}
      {hasHeroImage ? (
        <div
          className="absolute left-0 right-0 bottom-0 overflow-hidden"
          style={{ top: "calc(-1 * env(safe-area-inset-top))" }}
        >
          <HeroPhoto className="object-cover um-kenburns" />
          <span className="um-hero-drift" aria-hidden="true" />
        </div>
      ) : (
        <div className="absolute inset-0 bg-[#0A0A0B]" />
      )}

      {/* Tint + deep vignette so the gold and text always read cleanly */}
      <div
        className={`absolute inset-0 ${overlay.className}`}
        style={overlay.style}
      />
      <div
        ...
        style={{
          background:
            "radial-gradient(120% 90% at 50% 35%, transparent 0%, rgba(0,0,0,0.6) 100%)",
        }}
      />
      <HeroFadeOut color={nextColor} />

      {/* Soft pulsing gold light behind the name */}
      <span className="um-glow" aria-hidden="true" />

      {/* Thin gold inset frame */}
      <div className="pointer-events-none absolute inset-4 md:inset-6 border border-gold-soft" />

      {/* Editorial vertical edge labels */}
      {business.city && (
        <span
          className="um-vlabel um-in hidden md:block absolute left-9 top-1/2 -translate-y-1/2 text-[9px] tracking-[0.5em] uppercase text-white/30 font-thin z-10"
      ...
      {business.tagline && (
        <span
          className="um-vlabel um-in hidden md:block absolute right-9 top-1/2 -translate-y-1/2 text-[9px] tracking-[0.5em] uppercase text-gold-70 font-thin z-10"
      ...
      <div className="relative z-10 px-6 text-center max-w-3xl mx-auto">
        {/* Frosted-glass box — inset-x-0 matches the max-w-3xl container width
            so the business name is always fully covered */}
        {hasBackdrop && (
          <div
        ...
        <span
          className="um-in block text-xs tracking-[0.6em] uppercase text-white/70 mb-8"
          style={{ animationDelay: "0.15s" }}
        >
          Bun venit
        </span>

        {/* Fade on the <h1>, metallic sheen on the inner span — kept on separate
            elements because both set the CSS `animation` shorthand. */}
        <h1 className="um-namein font-script text-6xl sm:text-7xl md:text-[8.5rem] leading-[0.95]">
          <span className="gold-sheen">
            <RichInline text={business.name} />
          </span>
        </h1>

        {/* The long "————◆————" divider, drawn in after the name */}
        <div
          className="um-in flex items-center justify-center gap-4 my-8"
          style={{ animationDelay: "0.85s" }}
        >
          <span className="um-divider um-grow w-20 md:w-32" />
          <span className="um-divider-node shrink-0" />
          <span className="um-divider um-grow w-20 md:w-32" />
        </div>

        <p
          className="um-in text-xs md:text-sm tracking-[0.5em] uppercase text-white/80 mb-12 font-light"
      ...
      {/* Elegant drawn-in scroll cue — a hairline with a falling gold dot */}
      <a
        href="#services"
        className="group absolute bottom-9 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 z-10"
        aria-label="Scroll"
      >
        <span
          className="um-in text-[9px] tracking-[0.4em] uppercase text-white/40 group-hover:text-gold transition-colors"
          style={{ animationDelay: "1.4s" }}
        >
          Scroll
        </span>
        <span className="relative block">
          <span className="um-scrollline block" />
          <span className="um-scrollline-dot absolute left-1/2 top-0 -translate-x-1/2" />
        </span>
      </a>
    </section>
  );
...
export default function Hero() {
  ...
  if (v === "ultramodern") return <HeroUltramodern />;
  return <HeroClassic />;
}
